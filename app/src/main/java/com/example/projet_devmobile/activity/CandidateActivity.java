...
public class CandidateActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        candidatDataViewModel.setUserId(getIntent().getStringExtra("identifiant"));
        getLocalisation();

        // Initialization of the navigation bare
        NavigationLayout navigationBare = new NavigationLayout(this);
        navigationBare.setFunctionToAccountButton(NavigationLayout.CANDIDATE,fragmentManager);
        navigationBare.setFunctionToSearchButton(v ->
                fragmentManager.beginTransaction()
                .replace(R.id.menuContent, FilterFragment.newInstance(FilterFragment.SEARCH, null))
                .commit());
        LinearLayout navigationLayout = findViewById(R.id.navagationLayout);
        navigationLayout.addView(navigationBare);

        // Lunch the menu for candidates
        fragmentManager.beginTransaction()
                .replace(R.id.menuContent, new CandidatMenuFragment())
                .commit();
    }
